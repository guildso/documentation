...
Here you will learn how to create your own SPL token to reward users with your own token 🔥 We are going to use the [The Solana Program Library](https://spl.solana.com/) (SPL) to create this token.

## 📋 Prerequisites

- A new Linux server, for this example we will use Ubuntu 21.10 with at least 2GB of RAM. If you wish to use this same server to host your Laravel application, you may want to check out [Larasail](https://github.com/thedevdojo/larasail)
- A small amount of SOL to create your new token [(Buy SOL Here)](https://www.coinbase.com/join/iliev_pt)
...
After that's installed, you may be prompted to run an `export` command, so that you could add the solana binary to your `PATH`.

Next, you can confirm that you have the desired version of solana installed by running:
...
This new wallet will be created to hold your SOL, wich is needed to pay the small fee for creating a new token and making your transactions.
...
In this step we'll be adding some SOL to your wallet. This is needed for small transaction fees when sending/transferring/creating tokens. Go to your favorite cryptocurrency exchange and buy some SOL.
...
We will also need to have rust installed on our machine to run a few commands, including the `spl-token` program. You can install Rust by running the following command:
...
Once you've installed Rust, you may need to run the command `export` in order to add the Rust binary to your `PATH`. Alternatively, you can log out and log back in to your server to make sure the Rust binary is added to your `PATH`.

### Installing the `spl-token-cli`

> Only on Ubuntu Machine: The `spl-token-cli` is a command line interface to the SPL program. Before you can use the `spl-token-cli` you may need to install the following dependencies: `sudo apt install libudev-dev libssl-dev pkg-config build-essential`

To install the `spl-token-cli` by running the following command:
...
Lastly, you can send tokens to other accounts. Let's send some tokens to another account, for example you could create a new wallet by using the [Phantom Wallet](https://phantom.app/)

Once you have installed the Phantom Wallet browser extension, it will create a new phantom wallet with a new address.

Then you can send tokens to your phantom wallet by running the following command:
...
## ✅ Add Your Token to the Solana Token List

Your token has now been created and you can search for it in the [Solana Blockchain Explorer](https://explorer.solana.com/); however, you will probably not see much information about your new token in the explorer until you add it to the Solana Token List. In this step we will show you how to add it to the list.

Before moving on with these steps there are a few things you need to have before getting started:
...
### Fork the Solana Token List Repository
...
In a few hours, your token will be added to the list and you can see it in the [Solana Token Explorer](https://explorer.solana.com/tokens)



## 🛠 Troubleshooting

## Server less than 1GB of RAM
